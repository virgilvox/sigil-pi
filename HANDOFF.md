...
| VIDEOS / IMAGES | `videos` / `images` | **Swipe-through galleries** for whatever is in `public/videos` / `public/images` — auto-detected at runtime (drop a file, refresh, it appears). **The media is committed to the repo** so it deploys to the Pi via `git pull`. One `MediaViewerGame.vue` (`kind` prop) renders **only the current item** (keyed by url so swiping releases the previous element's memory). Videos autoplay with audio + mute; a **FILL/FIT** button toggles crop-to-circle vs contain; swipe or arrows page. See below. |
...
- **Media is committed to the repo** (`public/videos`, `public/images`) so it travels to the Pi via `git pull` — the apps are populated out of the box. Adding media = drop files in + commit them; only `.DS_Store` is ignored. Heads-up: the repo is **public**, so committed media is publicly downloadable. `SIGIL_MEDIA` still overrides the base dir if you'd rather keep a large/private library off-repo (drop it there on the Pi and point the env var at it — no rebuild needed).
